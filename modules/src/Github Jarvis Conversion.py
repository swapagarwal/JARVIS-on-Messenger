...
meter = {'meter': 1, 'kilometer': 0.001, 'centimeter': 100,
       'millimeter': 1000, 'micrometer': 1000000, 'nanometer': 10**9,
       'mile': 0.0006213689, 'yard': 1.0936132983, 'foot': 3.280839895,
       'inch': 39.37007874, 'light year': 1.057008707*10**(-16)}

celcius = {'celcius': 1, 'kelvin': 274.15, 'farenheit': 33.8}

squareMeter = {'square meter': 1,'square kilometer': 0.001**2, 
             'square centimeter': 100**2,'square millimeter': 1000**2,
...
gram = {'kilogram': 0.001, 'gram': 1, 'milligram': 1000,'metric ton': 0.000001,
        'long ton': 9.842073304*10**(-7), 'short ton': 0.0000011023,
         'pound': 0.0022046244, 'ounce': 0.0352739907, 'carrat': 5,
        'atomic mass unit': 6.022136652*10**(23)}
...
abbreviation = {'m': 'meter', 'km': 'kilometer', 'cm': 'centimeter','mm': 'millimeter',
                'nm': 'nanometer','ft': 'foot','c': 'celcius','f': 'farenheit',
              'k': 'kelvin','sq m': 'square meter','sq km': 'square kilometer',
              'sq ft': 'square foot','yd': 'yard', 'sq yard': 'square yard',
              'l': 'liter','ml': 'milliliter','g': 'gram',
                'kg': 'kilogram','mg': 'milligram','ton': 'metric ton','lb': 'pound',
                'amu': 'atomic mass unit','s': 'second','min': 'minute','hr': 'hour',
              'ms': 'millisecond','ns': 'nanosecond','ps': 'picosecond','d': 'day'}
...
def process(input, entities):
    ...
    try:
        ...
        if unit2 in meter.keys():
            output['output'] = TextTemplate(linearConversion(float(number),unit1.lower(),unit2)).get_message()        
        
        if unit2 in squareMeter.keys():
            output['output'] = TextTemplate(areaConversion(float(number),unit1.lower(),unit2)).get_message()
            
        if unit2 in cubicMeter.keys():
            output['output'] = TextTemplate(volumeLengthConversion(float(number),unit1.lower(),unit2)).get_message()        
        
        if unit2 in liter.keys():
            output['output'] = TextTemplate(volumeConversion(float(number),unit1.lower(),unit2)).get_message()
        
        if unit2 in gram.keys():
            output['output'] = TextTemplate(weightConversion(float(number),unit1.lower(),unit2)).get_message()
        
        if unit2 in minute.keys():
            output['output'] = TextTemplate(timeConversion(float(number),unit1.lower(),unit2)).get_message()
                                                                
        if unit2 in celcius.keys():
            output['output'] = TextTemplate(temperatureConversion(float(number),unit1.lower(),unit2)).get_message() 
            
        output['input'] = input
        output['success'] = True
    except:
        error_message = 'I couldn\'t convert between those parameters.'
