...
def process(input, entities):
	...
	try:
		if entities != {}:
			...
			weather_data = r.json()
			location_display_name = location_data[0]['display_name']
		else:
			r = requests.get('http://ipinfo.io/json')
			location_data = r.json()
			r = requests.get('http://api.openweathermap.org/data/2.5/weather?lat='+ location_data['loc'].split(',')[0] + '&lon='+ location_data['loc'].split(',')[1] + '&units=metric&appid=' + OPEN_WEATHER_MAP_ACCESS_TOKEN)
			weather_data = r.json()
			location_display_name = location_data['region'].title()

		output['input'] = input
		temperature_in_fahrenheit = weather_data['main']['temp'] * 1.8 + 32
		degree_sign = u'\N{DEGREE SIGN}'
		output['output'] = TextTemplate('Location: ' + location_display_name + '\nWeather: ' + weather_data['weather'][0]['description'] + '\nTemperature: ' + str(weather_data['main']['temp']) + ' ' + degree_sign + 'C / ' + str(temperature_in_fahrenheit) + ' ' + degree_sign + 'F\n- Info provided by OpenWeatherMap').get_message()
		output['success'] = True
