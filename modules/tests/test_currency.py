...
def test_currency():
    ...
    assert('currency' == modules.process_query('convert 5 HKD to USD')[0])
    assert('currency' == modules.process_query('convert 100 USD to INR')[0])
    assert('currency' == modules.process_query('how much is 50 USD in EUR')[0])